#!/usr/local/bin/perl
# $Id: qcCompare.pl,v 1.1.2.1 2005-10-07 20:45:42 catmandew Exp $
#
...
Compare two caqc-generated qc files
    ...
    file1       The 'reference' qc filename

    file2       The first 'query' qc filename
...
######################################################################
my $printedHeader = 0;
my $header = "Category Entry/Field: RefVal QueryVal Delta PctDelta";
...
for(my $i = 1; $i <= $#files; $i++)
{
  ...
  foreach my $key (keys(%f1data))
  {
    ...
    if(index($key, "=") > -1)
    {
      ...
      foreach my $keyItem (keys(%{$f1data{$key}}))
      {
    ...
    {
      foreach my $keyVar (keys(%{$f1data{$key}}))
      {
