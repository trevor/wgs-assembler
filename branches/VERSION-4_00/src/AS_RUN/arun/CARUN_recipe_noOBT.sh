...
$CA_BIN/overlap -M $MEMORY -t 2 -h 1-$HIFRAGID -r 0-$HIFRAGID \
  -k ${KMER} -k $WORKDIR/0-preoverlap/$PREFIX.${KMER}mers${NMER}.fasta \
  -o $PREFIX.ovb \
...
$CA_BIN/cgw -y -N 3 -c -j $ASTATLOW -k $ASTATHIGH -r 5 -s 0 \
  -m $AS_CGW_SAMPLE_SIZE \
  -S 0 -G -z \
  -R $MAXCHKPT   \
  -g $WORKDIR/$PREFIX.gkpStore   \
...
#> Generate QC report
cd $WORKDIR
$CA_BIN/caqc.pl $GENOMELENGTH -metric $PREFIX.asm > caqc.err 2>&1
